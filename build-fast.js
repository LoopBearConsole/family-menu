...
const lite = dishes.map((d) => ({
  ...
  tagline: (d.tagline || '').slice(0, 36),
  img: d.img || null,
}));

// 2) per-dish detail files (lazy)
for (const d of dishes) {
  ...
  fs.writeFileSync(path.join(DETAIL, d.id + '.json'), JSON.stringify(detail));
}

// 3) core.js (no giant DISHES array)
const core = `// 老刘小炒 · 加速核心（轻量）
...
fs.writeFileSync(
  path.join(ROOT, 'dishes-lite.js'),
  'const DISHES_LITE=' + JSON.stringify(lite) + ';\n'
);

console.log('lite KB', (Buffer.byteLength(JSON.stringify(lite)) / 1024).toFixed(1));
console.log('details', dishes.length);
console.log('core.js + dishes-lite.js written');
